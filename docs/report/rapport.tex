...
Un certain nombre de tâches supplémentaires sont arrivées ensuite ainsi qu'un redécoupage en sous-tâches pour l'implémentation. Le temps estimé a donc été augmenté une fois que nous avions une vision plus complète sur le travail à effectuer (analyse des besoins et conception terminées).
...
If he decides to compute the route and everything is fine, the program will be in NothingSelectedState, where you just have the map with the route computed but you don't have any node selected on the map or on the tree list. Here, the user has two choices:\\
...
Le \bf{ContextManager} a deux rôles majeurs qui lui valent d'implémenter deux patrons de conception qui sont le patron \bf{État} et le patron \bf{Commande}. Il a tout d'abord pour rôle d'entretenir la machine à états globale de l'application, c'est-à-dire de garantir à chaque instant qu'une commande utilisateur déclenchera les bons traitements dans l'application et donc les bonnes réactions de l'interface utilisateur et du modèle. Son second rôle est d'entretenir un historique des modifications effectuées de sorte à ce que l'utilisateur puisse défaire et refaire les dernières actions qu'il a effectué. Ce contrôleur assure à l'utilisateur que l'action qu'il veut effectuer a du sens dans le processus global que déroule l'application. Il est dans cette optique étroitement lié à l'\bf{UIManager} qui lui fait parvenir les commandes de l'utilisateur. Il est, à ce stade, important de préciser que ces différents composants implémentent tous le patron \bf{Singleton} afin de garantir l'unicité de chaque interface et par conséquent l'unicité du modèle, de l'état global de l'application et de l'historique des commandes effectuées.\\
...
The main goal of the application is to figure out what is the best route to deliver in the shortest times each customer, thanks to a loaded planning. The previous sequence diagram shows us the interactions between the different parts of the model used to compute this route.\\
...
\section{Application mockup \it{(EN)}}
\label{sec:mockup}

Before we started to implement the user interface, we did first the following mockup, setting the main actions and controls of the main window. 

\begin{center}
\begin{figure}[H]
\centering
\includegraphics[scale=0.35,angle=0]{figures/mockup.png}
\caption{Initial mock-up}
\end{figure}
\end{center}
...
The main difference that was introduced during the developpement was the creation of the \textit{PlanningComputeRouteWorker} that allow us to launch the route computation in a different thread. Indeed, in \textit{Swing} or in \textit{JavaFX}, everything that is launched from the UI stays in the UI thread by default. This can lead to a "buggy" interface, that seems to freeze during long computation. This also permits to interrupt the process.\\

As a result we also introduce thread creation during the map and planning loading. This allows to change the appearance of the cursor during the loadings.\\

For convenience purposes, double sided access of attribute from different parts of the model were added. For example, we didn't plan to make a getter for the \textit{Planning} and a getter for the \textit{Planning} in the \textit{Map}.\\

Otherwise, some additional methods have been added to make the view access information more easily. Indeed, the method \textit{getAssociatedTimeSlots} can now be found in the \textit{Arc} object as that was much more convenient to color the arcs in the view.
...
The first thing we can see is that there are lots of method we hadn't planned to introduce during the conception phase. We have also changed the way of managing the nodes, by creating a \textit{NodeViewShapeFactory} in order to make the creation of nodes more generic and robust to changes.\\
...
Very few modifications were added in the controller package. Some classes were added to launch the application such as \textit{AppMediator}. But this is mainly due to javafx behaviour concerning the launching of the application and the fact that it was not possible to get the intance of the launch application when it was launched.\\

Few methods were added in the \textit{UIManager} in order to allow the other controllers and actions to gain access to more control on the view.
