...
public class Planning implements Publisher {
    ...
    private void initNodesTimeSlot() {
        map.resetNodes();
        timeSlots.stream().forEach((ts) -> ts.getDeliveries().stream().forEach(Delivery::updateNode));
    }
    ...
    public List<Delivery> getDeliveries() {
        ...
        timeSlots.stream().forEach((ts) -> deliveries.addAll(ts.getDeliveries()));
        return deliveries;
    }
    ...
    public void notifySubscribers() {
        subscribers.stream().forEach((subscriber) -> subscriber.update(this, null));
    }
